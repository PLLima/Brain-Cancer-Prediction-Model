...
# Run from anywhere: set the working directory to the project root (folder containing data/)
setwd({ .r <- normalizePath(getwd(), winslash = "/", mustWork = FALSE); while (!dir.exists(file.path(.r, "data")) && dirname(.r) != .r) .r <- dirname(.r); .r })
...
saveRDS(list(cv=results, test=test_results, cms=cms),
        "results/nb17_class_weights_results.rds")
