...
# --------------------------------------------------------------------------
# ---- Locate project root (portable, depth-independent) ----
find_project_root <- function() {
  ...
  repeat {
    if (dir.exists(file.path(d, "data"))) return(d)
    parent <- dirname(d)
    if (parent == d) stop("Project root (with data/) not found")
    d <- parent
  }
}
...
FIG_DIR     <- file.path(ROOT, "figures"); dir.create(FIG_DIR, showWarnings = FALSE, recursive = TRUE)
...
saveRDS(fit_final,  file.path(out_dir, "sgcca_fit_final.rds"))
saveRDS(cv_out,     file.path(out_dir, "sgcca_cv.rds"))
saveRDS(fit_stable, file.path(out_dir, "sgcca_stable.rds"))
saveRDS(boot_out,   file.path(out_dir, "sgcca_bootstrap.rds"))

# Plots PDF
pdf(file.path(FIG_DIR, "sgcca_plots.pdf"), width = 10, height = 6)
plot(cv_out, cex = 1.5)
...
cat("\n=== TERMINÉ ===\n")
cat(sprintf("Modèle final          : %s\n", file.path(out_dir, "sgcca_fit_final.rds")))
cat(sprintf("Plots                 : %s\n", file.path(FIG_DIR, "sgcca_plots.pdf")))
