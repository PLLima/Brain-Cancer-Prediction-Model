...
if (needs_data) {
  ...
  # Locate data/ by walking up from the current directory (depth-independent)
  d <- normalizePath(getwd(), winslash = "/", mustWork = FALSE)
  repeat {
    if (dir.exists(file.path(d, "data"))) { project_root <- d; break }
    parent <- dirname(d)
    if (parent == d) stop("Project root (with data/) not found from ", getwd())
    d <- parent
  }
...
# ---------- 6. Sauvegarde ----------
# Resolve <project_root>/results/ by walking up until data/ is found
.d <- normalizePath(getwd(), winslash = "/", mustWork = FALSE)
while (!dir.exists(file.path(.d, "data")) && dirname(.d) != .d) .d <- dirname(.d)
results_dir <- file.path(.d, "results")
dir.create(results_dir, showWarnings = FALSE, recursive = TRUE)
out_path <- file.path(results_dir, "nb14c_results.rds")
saveRDS(list(
